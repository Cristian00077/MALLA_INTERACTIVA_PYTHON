...
class MallaCurricular:
    ...
    def aprobar_semestre(self, semestre):
        """Aprueba todas las materias de un semestre completo"""
        materias_semestre = [materia for materia, _ in self.malla[semestre]]
        
        # Aprobar todas las materias del semestre sin verificar prerequisitos
    ...
    def toggle_materia(self, materia, boton):
        ...
        if materia in self.materias_aprobadas:
            # Verificar si otras materias dependen de esta
            dependientes = [m for m, prereqs in self.prerequisitos.items() 
                          if materia in prereqs and m in self.materias_aprobadas]
            
            if dependientes:
                messagebox.showwarning(
                    "No se puede desmarcar",
                    f"La materia '{materia}' es prerequisito de:\n" + 
                    "\n".join(f"• {dep}" for dep in dependientes)
                )
                return
